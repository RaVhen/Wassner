...
void correctPosition(Node *nodes, int nodeCt){
	...
	//printf("DEBUG DELTA CENTER: X:%lf   Y:%lf", deltaX, deltaY);
...
void affichage(sfRenderWindow *fenetre, Node *nodes, int nodeCt, Link links) {
	...
	sfShape ** accelerations = malloc(sizeof(sfShape*)*nodeCt);
	sfShape ** vitesses =  malloc(sizeof(sfShape*)*nodeCt);
	...
	/* TEST FEATURE : gravity center */
	/*
	float gravityX, gravityY, surface;
	gravityX = 0; gravityY=0;
	surface=0;
	
	for(i=0;i<nodeCt-1;i++){
		surface += (nodes[i].posX * nodes[i+1].posY) - (nodes[i+1].posX * nodes[i].posY);
		gravityX+= (nodes[i].posX + nodes[i+1].posX) * (nodes[i].posX * nodes[i+1].posY - nodes[i+1].posX * nodes[i].posY);
		gravityY+= (nodes[i].posY + nodes[i+1].posY) * (nodes[i].posX * nodes[i+1].posY - nodes[i+1].posX * nodes[i].posY);
	}
	//last iterations are looping with first element
	surface += (nodes[i].posX * nodes[0].posY) - (nodes[0].posX * nodes[i].posY);
	gravityX+= (nodes[i].posX + nodes[0].posX) * (nodes[i].posX * nodes[0].posY - nodes[0].posX * nodes[i].posY);
	gravityY+= (nodes[i].posY + nodes[0].posY) * (nodes[i].posX * nodes[0].posY - nodes[0].posX * nodes[i].posY);
	
	surface*= 0.5; //finish surface formula
	//finish formulas
	gravityX *= 1/(6*surface);
	gravityY *= 1/(6*surface);
	
	
	sfShape *barycentre = malloc(sizeof(sfShape*));

	barycentre = sfShape_CreateCircle(gravityX, gravityY, 5, rouge, 1, gris);
	*/
	...
	*/
	...
	sfShape *circleXmax = malloc(sizeof(sfShape*));
	sfShape *circleXmin = malloc(sizeof(sfShape*));
	sfShape *circleYmax = malloc(sizeof(sfShape*));
	sfShape *circleYmin = malloc(sizeof(sfShape*));
	...
	sfShape *centre = malloc(sizeof(sfShape*));
	...
	//TEST FEATURE BARYCENTRE:
	//	sfRenderWindow_DrawShape(fenetre,barycentre);
	...
	//DEBUG-VECTORS
	for(i=0;i<nodeCt;i++){ // Tracer les vecteurs vitesse/acceleration
